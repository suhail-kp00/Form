...
export function isAuthenticated(request) {
  ...
  return payload !== null && payload.auth === true && payload.user === "admin";
}

export function createSessionToken() {
  return sign({ auth: true, user: "admin", exp: Date.now() + COOKIE_MAX_AGE * 1000, nonce: randomBytes(8).toString("hex") });
}
