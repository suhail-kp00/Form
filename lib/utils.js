...
export function sanitizeFormPayload(payload) {
  ...
  if (!questionsInput.length) {
    throw new Error("Add at least one question to create the form.");
  }
  ...
  const questions = questionsInput.map((question, index) => {
    ...
    if (!label) {
      throw new Error(`Question ${index + 1} needs a label.`);
    }

    if (isOptionQuestion(type) && options.length < 2) {
...
export function normalizeAnswers(form, answersInput) {
  ...
  for (const question of form.questions) {
    ...
    if (value && isOptionQuestion(question.type) && !question.options.includes(value)) {
      throw new Error(`"${question.label}" contains an invalid option.`);
    }

    if (value && question.type === "number" && Number.isNaN(Number(value))) {
      throw new Error(`"${question.label}" must be a valid number.`);
    }

    if (value && question.type === "email" && !value.includes("@")) {
      throw new Error(`"${question.label}" must be a valid email address.`);
    }
...
export function buildExcelWorkbookXml(form, responses) {
  ...
  const dataRows = table.rows
    .map((row) => {
      const cells = [
        `<Cell><Data ss:Type="String">${escapeXml(row.submittedAtLabel)}</Data></Cell>`,
        ...form.questions.map((question) => {
          const value = row.cells[question.id] ?? "";
