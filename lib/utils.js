...
// ---- CSV export (replaces XML/XLS — opens in Excel with no warnings) ----
export function buildCsvExport(form, responses) {
  ...
  const header = table.columns.map((col) => escapeCsv(col.label)).join(",");

  const rows = table.rows.map((row) => {
    const cells = [
      escapeCsv(row.submittedAtLabel),
      ...form.questions.map((q) => escapeCsv(row.cells[q.id] ?? ""))
    ];
    return cells.join(",");
  });
