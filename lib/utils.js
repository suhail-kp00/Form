...
const ALLOWED_TYPES = new Set([
  ...
  "dropdown", "date", "number", "email", "image", "phone", "address"
]);
...
export function sanitizeFormPayload(payload) {
  ...
  return { title, description, questions, isBot };
}
...
export function normalizeAnswers(form, answersInput) {
  ...
  for (const question of form.questions) {
    ...
    const value = sanitizeText(rawValue, question.type === "address" ? 500 : 500);
    if (question.required && !value) throw new Error(`"${question.label}" is required.`);
    ...
    if (value && question.type === "phone" && !/^[6789]\d{9}$/.test(value)) throw new Error(`"${question.label}" must be a valid 10-digit number starting with 6, 7, 8, or 9.`);
