...
try {
  database.exec(`ALTER TABLE forms ADD COLUMN is_bot INTEGER NOT NULL DEFAULT 0`);
} catch {
  // Column already exists, ignore
}

const insertFormStatement = database.prepare(`
  INSERT INTO forms (public_id, title, description, schema_json, created_at, is_bot)
  VALUES (?, ?, ?, ?, ?, ?)
`);
...
export function createForm({ title, description, questions, isBot = false }) {
  const createdAt = new Date().toISOString();
  const publicId = createPublicId();
  const result = insertFormStatement.run(publicId, title, description, JSON.stringify(questions), createdAt, isBot ? 1 : 0);
